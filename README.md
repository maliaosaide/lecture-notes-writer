...
│   ├── 讲座讲稿.pdf
│   ├── 讲座讲稿.pdf-<hash>/          # MinerU 转换文件夹
│   │   ├── full.md
│   │   ├── MinerU_markdown_*.md
│   │   ├── images/
│   │   ├── *_content_list.json
│   │   └── *_origin.pdf
│   └── recording_*.txt                # 可选；如果提供 URL，则不需要本地转写文件
│
├── 笔记/                              # 【输出】skill 自动创建，每讲座独立子目录
└── temp/                              # 【中间产物】skill 自动创建，URL 抓取的录音落地
```

然后在 Claude Code 中说：

```text
请使用 lecture-notes-writer，根据 `原始资料/讲座讲稿.pdf`、`原始资料/讲座讲稿.pdf-<hash>/` 和录音转文字 URL（或 `原始资料/录音转文字.txt`）撰写一份详细讲座笔记。
...
```text
工作目录/
├── 原始资料/                         # 输入：用户提供的 PDF + MinerU + 录音
│   ├── 讲座A.pdf
│   ├── 讲座A.pdf-<hash>/
│   └── recording_*.txt
├── 笔记/                             # 输出：每讲座独立子目录，避免 pdf_pages 互相覆盖
│   └── <讲座名>/
│       ├── 笔记.md
│       ├── 论文引用验证补充.md        # 可选
│       └── pdf_pages/
...
```bash
# PDF 转页面截图（PDF 从 原始资料/ 读，输出到 笔记/<讲座名>/pdf_pages）
python scripts/pdf_to_pages.py "原始资料/讲稿.pdf" "笔记/<讲座名>/pdf_pages"

# 提取讲稿章节页码
python scripts/extract_headings.py "原始资料/<MinerU文件夹>/*_content_list.json"

# 自动化审查（主观人称/图片路径/数字一致性/附录完整性）
python scripts/audit_notes.py "笔记/<讲座名>/笔记.md" \
  --content-list "原始资料/<MinerU文件夹>/*_content_list.json" \
  --lecturer "讲者姓名"

# 录音覆盖率检查（检查录音洞察是否完整融入笔记）
python scripts/coverage_check.py "笔记/<讲座名>/笔记.md" "temp/recording_<讲座名>.txt"

# 验证图片与内容一致性
python scripts/verify_image_consistency.py "笔记/<讲座名>/笔记.md" "原始资料/<MinerU文件夹>/*_content_list.json"

# 提取论文引用清单（Step 5.5 用）
python scripts/verify_citations.py "笔记/<讲座名>/笔记.md"
```
...
> 以下示例取自真实讲座笔记（谢吉宝《从 Code Agent 到 General Agent》），展示 skill 的图文对应规则、完整记录原则与题外话格式。

### 示例 1：单图对应单论点（基本模式）

![讲稿第 9 页：智能体能力坐标系](examples/sample_page_009_coordinate.png)

*↑ 讲稿原页（PDF 第 9 页）· 智能体能力坐标系（推理深度 × 工具能力）*
...
### 示例 2：一个论点跨多页（v1.2.0 图片插入策略）

讲者讲"学界关键工作与业界产品形态"这一论点时翻过两张页面（学界工作页 + 业界产品页），笔记就该**两张都插**——它们是同一论点的两个侧面，不是"重复"。

![讲稿第 16 页：学界关键工作（一）](examples/sample_page_016_academic_work.png)

*↑ 讲稿原页（PDF 第 16 页）· 学界关键工作（一）：ReAct / Toolformer / Voyager*

![讲稿第 18 页：业界产品形态对比](examples/sample_page_018_industry.png)

*↑ 讲稿原页（PDF 第 18 页）· 业界 AI 编程助手核心维度对比*

对应笔记节选（**完整记录**：两组工作 + 论文引用块 + 多条题外话）：

> 学界关键工作分为两组。第一组（基础范式）：**ReAct**（Yao et al., Princeton, ICLR 2023）将推理与行动统一为"Thought → Action → Observation"三段式交错生成范式，在 ALFWorld 和 WebShop 上分别取得 34% 和 10% 的绝对成功率提升；**Toolformer**（Schick et al., Meta AI, NeurIPS 2023）让模型通过自监督学习掌握何时调用工具、调用哪个工具、传什么参数，仅需少量演示即可使用计算器、搜索引擎、QA 等 5 类工具；**Voyager**（Wang et al., NVIDIA, 2023）作为首个基于 LLM 的终身学习具身智能体，在 Minecraft 中通过 GPT-4 黑盒 API（无微调）实现开放探索，比 AutoGPT/ReAct/Reflexion 多发现 **3.3×** 物品、**15.3×** 更快解锁关键技术树。第二组（代码智能体深化）：**SWE-Agent**（Yang et al., Princeton, NeurIPS 2024）提出 Agent-Computer Interface（ACI）——为 LLM 专门设计的交互层（类比人用 IDE，但为 token 推理优化），在 SWE-bench 上达 **12.5% pass@1**、HumanEvalFix 上 **87.7%**；**AutoCodeRover**（Zhang et al., NUS, ISSTA 2024）将 LLM 与结构感知的 AST 代码搜索结合（`search_class` 等专用 API），可选叠加 SBFL 故障定位，SWE-bench-lite 上达 **19% pass@1**，平均 **$0.43 / ~4 分钟** 每题；**OpenHands / Aider / Cline** 等开源探索覆盖从终端补全到全栈自动修复的多样化实践。
>
> **参考论文**：
>
...
> - Zhang et al. *"AutoCodeRover: Autonomous Program Improvement"*. ISSTA 2024. [arXiv:2404.05427](https://arxiv.org/abs/2404.05427)
>
...
>
> 💡 **题外话（研究者观察）**：Toolformer 论文 abstract 强调"自监督学习用工具"，但论文第 3 节的训练流程有一个关键设计——模型先在原始文本上**采样潜在工具调用**，执行后**只保留那些能降低后续 token 损失的调用**作为训练数据。这意味着模型学到的不是"何时用工具"的规则，而是"用工具后预测更准"的统计信号。论文 6.4 节 ablation 显示，去掉这个"有用性筛选"机制后，模型会出现大量无用工具调用——这是"自监督有效性"的核心证据。
>
> 💡 **题外话（研究者观察）**：Voyager 论文 abstract 强调"终身学习"和"开放探索"，但论文第 4 节的技能库设计有几个值得注意的细节：(1) 技能以**可执行 JavaScript 代码**形式持久化（基于 Mineflayer API），而非自然语言描述——技能可直接调用而非"再解释"；(2) 技能库通过 **embedding 向量按语义相似度检索**，并显式支持组合式调用——把"程序化知识的复用"转化为"向量检索"问题；(3) 论文 5.3 节实验显示，**技能库可零样本迁移到全新 Minecraft 世界**——这是论文本身提供的"技能可迁移性"证据。

> **说明**：实际笔记中本节还有 SWE-Agent / AutoCodeRover 两条题外话，因篇幅原因此处省略——完整记录原则要求所有讲者展开过的论文题外话都保留。
...
└── examples/                          # README 展示用示例图
    ├── sample_page_009_coordinate.png  # 示例 1：能力坐标系页
    ├── sample_page_016_academic_work.png # 示例 2：学界关键工作页
    └── sample_page_018_industry.png    # 示例 2：业界产品形态页（同一论点的第二张）
```
...
A：不会。每场讲座在 `笔记/<讲座名>/` 下有独立子目录（`笔记/<讲座名>/笔记.md` 和 `笔记/<讲座名>/pdf_pages/`），互不覆盖。多讲座共享 `原始资料/`（不可变输入）和 `temp/`（一次性中间产物）。
...
- **目录结构重组为三层**：`原始资料/`（输入，多讲座共享）+ `笔记/`（输出，每讲座独立子目录）+ `temp/`（URL 抓取的中间产物）。避免讲稿散落、防止 `pdf_pages/` 互相覆盖、便于一次性清理中间文件。
- **录音抓取改为分级回退表**：优先 `mcp__grok-search__web_fetch` → 次选 `mcp__web-reader__webReader` → 兜底 `mcp__chrome-devtools__*`。明确"不要一上来就开浏览器"——chrome-devtools 是最后兜底而非首选。
- **新增"逐字稿完整性审查"强制流程**：抓回的原文必须经过数据量（字符数 vs 录音分钟数）、内容性质（是否混入 AI 摘要特征）、覆盖完整性（首尾时间戳、说话人切换数）三项校验，确认是纯原文逐字稿（非纪要/摘要/混合内容）才能进入 Step 2。
- **图片插入策略升级**：从"每章节 1-2 张代表页"改为"每个子论点都该配图"。新增 4 项判断问题（含核心原则"图必须服务于笔记论点结构"），明确"一个论点跨多页时全部都该插"，技术讲座不设上限。
- **质量自检清单新增 2 项**：图片充分性（不机械设上限）、逐字稿完整性已校验。
- **常见坑扩展至 10 条**：新增 #9 抓回未做完整性审查、#10 图片插入过度保守。
- **README 示例**：示例 2 扩展为"一论点跨多页"演示（补 sample_page_018_industry.png），同步展示完整论文引用块与多条题外话。
